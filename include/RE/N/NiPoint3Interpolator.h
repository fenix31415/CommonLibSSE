...
namespace RE
{
	...
	{
	public:
		inline static constexpr auto RTTI = RTTI_NiPoint3Interpolator;
		inline static constexpr auto Ni_RTTI = NiRTTI_NiPoint3Interpolator;

		~NiPoint3Interpolator() override = default;  // 00
		...
		NiPoint3             val;        // 18
		uint8_t              pad24[4];   // 24
		NiPointer<NiPosData> data;       // 28
		uint32_t             lastIndex;  // 30
		uint8_t              pad34[4];   // 34
	};
	static_assert(sizeof(NiPoint3Interpolator) == 0x38);
}
